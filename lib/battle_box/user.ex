defmodule BattleBox.User do
  ...
  def anon_human_user do
    ...
    {:ok, _user} =
      %__MODULE__{
        # :shrug:, -2 will work for now, eventually when auth is reworked this will be a nullable
        # field or stored on a different table
        github_id: -2,
        avatar_url: Routes.generated_avatar_url(Endpoint, :avatar, @anon_username),
        username: @anon_username
      }
      |> Repo.insert(conflict_target: :username, on_conflict: :nothing)

    Repo.get_by!(__MODULE__, username: @anon_username)
  end
